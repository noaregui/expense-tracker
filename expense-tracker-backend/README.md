...
<p align="center">
  Backend de una aplicación de seguimiento de gastos (Expense Tracker) construido con <a href="https://nestjs.com/" target="_blank">NestJS</a> y <a href="https://www.typescriptlang.org/" target="_blank">TypeScript</a>.
</p>
...
**Nota:** La persistencia de datos se realiza en memoria para simplificar la implementación. No se utiliza base de datos real.
...
<ul>
  <li>src/main.ts: punto de entrada de la aplicación, activa validaciones globales, CORS y Swagger.</li>
  <li>src/domain/: entidades del dominio (<code>Expense</code>).</li>
...
**Swagger**: está disponible en `/api-docs` para consultar documentación interactiva.
...
- **Repository pattern**: `ExpensesRepository` desacopla persistencia de la lógica de negocio.

- **Persistencia en memoria**: simplifica la implementación. Para producción se recomienda reemplazar `ExpensesRepository` por una base de datos real.

- **Endpoints REST**: cada operación del dominio tiene su caso de uso.

- **Validaciones automáticas**: con `class-validator` y `ValidationPipe` de NestJS.
- **Documentación interactiva**: Swagger `/api-docs` para explorar los endpoints y sus DTOs.
